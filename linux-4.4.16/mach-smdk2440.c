...
static struct s3c2410fb_mach_info smdk2440_fb_info __initdata = {
	.displays	 = &smdk2440_lcd_cfg[0], /* not constant! see init */
	.num_displays	 = 1,
	.default_display = 0,
...
static struct s3c24xx_led_platdata smdk2440_led4_pdata = {
	.name		= "led4",
	.gpio		= S3C2410_GPB(8),
	.flags		= S3C24XX_LEDF_ACTLOW | S3C24XX_LEDF_TRISTATE,
	.def_trigger	= "",
...
static struct platform_device smdk2440_led4 = {
	.name		= "s3c24xx_led",
	.id		= 4,
	.dev		= {
		.platform_data	= &smdk2440_led4_pdata,
	},
};
 ...
 *
 */
static char smdk2440_features_str[12] __initdata = "0tb";

static int __init smdk2440_features_setup(char *str)
{
	if (str)
		strlcpy(smdk2440_features_str, str, sizeof(smdk2440_features_str));
	return 1;
}

__setup("smdk2440=", smdk2440_features_setup);
...
static void __init smdk2440_parse_features(
		struct smdk2440_features_t * features,
	...
	while (*fp) {
		...
		case '0'...'9':	/* tft screen */
			if (features->done & FEATURE_SCREEN) {
				printk(KERN_INFO "SMDK2440: '%c' ignored, "
					"screen type already set\n", f);
			} else {
				int li = f - '0';
				if (li >= ARRAY_SIZE(smdk2440_lcd_cfg))
					printk(KERN_INFO "SMDK2440: "
...
{
	struct smdk2440_features_t features = { 0 };
	int i;

	printk(KERN_INFO "SMDK2440: Option string smdk2440=%s\n",
			smdk2440_features_str);

#if 1
    unsigned int oldval_gpfcon = *(volatile unsigned int *)S3C2410_GPFCON;
    unsigned int oldval_gpfup = *(volatile unsigned int *)S3C2410_GPFUP;
    unsigned int oldval_extint0 = *(volatile unsigned int *)S3C2410_EXTINT0;
    unsigned int oldval_eintmask = *(volatile unsigned int *)S3C2410_EINTMASK;
    unsigned int oldval_bwscon = *(volatile unsigned int *)S3C2410_BWSCON;

     /* Set GPF7 as EINT7 */
	*((volatile unsigned int *)S3C2410_GPFCON) = oldval_gpfcon & (~(3 << 14)) | (2 << 14);
	*((volatile unsigned int *)S3C2410_GPFUP) = oldval_gpfup | (1 << 7);
	/* EINT7 High level interrupt */
	*((volatile unsigned int *)S3C2410_EXTINT0 ) = (oldval_extint0 & (~(0x7 << 28))) | (0x1 << 28);
	/* Enable EINT7 */
	*((volatile unsigned int *)S3C2410_EINTMASK) = oldval_eintmask & (~(1<<7));
	/* Set GPA15 as nGCS4 */
	*((volatile unsigned int *)S3C2410_GPACON) |= 1 << 15;
	/* DM9000 width 16, wait enable */
	*((volatile unsigned int *)S3C2410_BWSCON) = oldval_bwscon & (~(0x7<<16)) | (0x5<<16);
	*((volatile unsigned int *)S3C2410_BANKCON4) = (1<<13) | (1<<11) | (0x6<<8) | (1<<6) | (1<<4) | (0<<2) | (0);
#endif
	/* Parse the feature string */
	smdk2440_parse_features(&features, smdk2440_features_str);
	...
	if (features.lcd_index != -1) {
		...
		smdk2440_fb_info.displays =
			&smdk2440_lcd_cfg[features.lcd_index];

		printk(KERN_INFO "SMDK2440: LCD");
		for (li = 0; li < ARRAY_SIZE(smdk2440_lcd_cfg); li++)
			if (li == features.lcd_index)
				printk(" [%d:%dx%d]", li,
					smdk2440_lcd_cfg[li].width,
					smdk2440_lcd_cfg[li].height);
			else
				printk(" %d:%dx%d", li,
					smdk2440_lcd_cfg[li].width,
					smdk2440_lcd_cfg[li].height);
	...
	i2c_register_board_info(0, smdk2440_i2c_devs,
				ARRAY_SIZE(smdk2440_i2c_devs));
